...
smoothing_methods_template = {
    "Bayesian": {
        "op_rw_b_eg0.txt": 0.0,
        "op_rw_b_eg02.txt": 0.2,
        "op_rw_b_eg04.txt": 0.4,
        "op_rw_b_eg06.txt": 0.6,
        "op_rw_b_eg08.txt": 0.8,
        "op_rw_b_eg1.txt": 1.0
    },
    "Dirichlet": {
        "op_rw_d_eg0.txt": 0.0,
        "op_rw_d_eg02.txt": 0.2,
        "op_rw_d_eg04.txt": 0.4,
        "op_rw_d_eg06.txt": 0.6,
        "op_rw_d_eg08.txt": 0.8,
        "op_rw_d_eg1.txt": 1.0
    }
}
...
jm_file_template = "op_rw_jm{:02d}_eg{}.txt"

# Load rw dataset
rw_file = "rw_processed.txt"
rw_pairs = []

with open(rw_file, "r") as f:
    for line in f:
        word1, word2, score = line.strip().split()  # Assuming space-separated
        rw_pairs.append((word1, word2, float(score)))
...
for idx, lambda_value in enumerate(jm_lambdas):
    ...
    for method, file_info in smoothing_methods.items():
        for filename, eig_weight in file_info.items():
            ...
            for word1, word2, human_score in rw_pairs:
...
# Save as a .png file
output_file = "spearman_correlation_rw_avg.png"
